...
#[derive(Clone)]
pub struct EditorAuthLayer {
    state: ServerState,
}

impl EditorAuthLayer {
    pub fn new(state: ServerState) -> Self {
        Self { state }
    }
}

impl<S> Layer<S> for EditorAuthLayer {
    type Service = EditorAuth<S>;
...
#[derive(Clone)]
pub struct EditorAuth<S> {
    inner: S,
    state: ServerState,
}

impl<S> Service<Request<Body>> for EditorAuth<S>
where
    S: Service<Request<Body>, Response = Response<Body>>,
{
    ...
    fn call(&mut self, req: Request<Body>) -> Self::Future {
        ...
        if let Some(name) = get_logged_in(&self.state, &jar) {
            if get_perms(&self.state, &name) >= Some(Perms::Editor) {
