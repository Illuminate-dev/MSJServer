...
use crate::{admin::ADMIN_EDIT_ARTICLE_TEMPLATE, *};
...
pub async fn post_edit_article(
    ...
    if let Some(mut article) = Article::get_article_by_uuid(form.uuid) {
        match form.action {
            EditArticleAction::Delete => {
                article.delete().expect("failed to delete file");
                Ok(Redirect::to("/admin"))
            }
            EditArticleAction::Edit => {
                if let Some((title, content)) = form.title.as_ref().zip(form.content.as_ref()) {
                    ...
                    Ok(Redirect::to(format!("/article/{}", form.uuid).as_str()))
                } else {
